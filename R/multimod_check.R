#' Multimodality test for post clustering variable involvement with intra-cluster multimodality checking
#'
...
#' cl <- hcl_fun(X)
#'plot(X, col=cl)
#'test_var1 <- test_multimod_ttest(X, g=1, k1=1, k2=2, cl = cl)
#'test_var2 <- test_multimod_ttest(X, g=2, k1=1, k2=2, cl = cl)

test_multimod_ttest <- function(X, g, cl, k1, k2){
  if (!is.matrix(X))
  ...
  dip_k1 <- dip.test(X[which(cl == k1), g])
  dip_k2 <- dip.test(X[which(cl == k2), g])

  ttest <- stats::t.test(X[cl==k1, g], X[cl==k2, g], alternative = "two.sided")

  if(dip_k1$p.value <0.05 | dip_k2$p.value < 0.05){
    result <- list(data_for_test = data,
                   stat_g = as.numeric(dip$statistic),
                   pval_dip = dip$p.value,
                   pval_ttest = ttest$p.value,
                   pval = max(ttest$p.value, dip$p.value),
                   pval_k1 = dip_k1$p.value,
                   pval_k2 = dip_k2$p.value,
                   correct = TRUE)
  }
  else{
    result <- list(data_for_test = data,
